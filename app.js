...
function audioMessageBox(label, audioUrl, statusText = "Zvukov\u00fd z\u00e1znam p\u0159ipraven") {
  return `
    <div class="audio-message-box terminal-panel">
      <div class="audio-control-row">
        <button class="primary-btn audio-listen-btn" type="button" data-audio-message ${audioUrl ? `data-audio-src="${escapeHtml(audioUrl)}"` : ""}>${escapeHtml(label)}</button>
        <button class="primary-btn audio-pause-btn" type="button" data-audio-pause disabled>Stopnout</button>
      </div>
      <div class="audio-visualizer" aria-hidden="true">
        <canvas data-audio-wave width="760" height="128"></canvas>
      </div>
      <input class="audio-scrubber" type="range" min="0" max="100" value="0" step="0.1" data-audio-scrub disabled aria-label="Posunout audio">
      <div class="audio-status" data-audio-status>${escapeHtml(statusText)}</div>
    </div>
  `;
}
...
function renderAudio(slide) {
  return `
    ${header("Audio stopa")}
    ${audioMessageBox(slide.listenButtonText || "Poslechnout vzkaz od historika Tom\u00e1\u0161e Havelky", slide.audioUrl ? resolveAssetUrl(slide.audioUrl) : "", "Vzorov\u00e1 zvukov\u00e1 stopa p\u0159ipravena")}
...
function renderInterlude(slide) {
  ...
  return `
    ...
    ${audioMessageBox("Poslechnout vzkaz od detektiva", slide.audioUrl ? resolveAssetUrl(slide.audioUrl) : "")}
    ${extraAudioMessages.map((audio) => audioMessageBox(audio.listenButtonText || "Poslechnout vzkaz", audio.audioUrl ? resolveAssetUrl(audio.audioUrl) : "")).join("")}
    ${slide.noticeText ? `
    ...
    ${renderCodeGate(slide)}
    <div class="actions ${slide.requiredCode ? "actions-after-code" : ""}"><button class="primary-btn" data-next ${isCodeGateUnlocked(slide) ? "" : "disabled"}>${escapeHtml(slide.buttonText || "Pokra\u010dovat")}</button></div>
  `;
}
...
function renderConditionalInterlude(slide) {
  ...
  return `
    ...
    ${audioMessageBox("Poslechnout vzkaz od detektiva", audioUrl)}
    ${slide.link ? `<div class="case-link-slot"><a class="case-link" href="${escapeHtml(linkUrl)}" target="_blank" rel="noreferrer">${escapeHtml(slide.link.label)}</a></div>` : ""}
    <div class="actions"><button class="primary-btn" data-next>Pokra\u010dovat</button></div>
  `;
}

function codeGateKey(slide) {
  return `codeGate-${slide.id}`;
}

function isCodeGateUnlocked(slide) {
  return !slide.requiredCode || state.answers?.[codeGateKey(slide)] === "unlocked";
}

function renderCodeGate(slide) {
  if (!slide.requiredCode) return "";
  const unlocked = isCodeGateUnlocked(slide);
  return `
    <div class="code-gate ${unlocked ? "is-unlocked" : ""}" data-code-gate>
      <button class="code-gate-trigger" type="button" data-code-trigger ${unlocked ? "disabled" : ""}>ZADAT K\u00d3D Z RE\u0160ER\u0160E</button>
      <div class="code-gate-entry ${unlocked ? "" : "hidden"}" data-code-entry>
        <input class="code-gate-input" data-code-input autocomplete="off" spellcheck="false" ${unlocked ? "disabled" : ""} placeholder="K\u00d3D">
        <button class="code-gate-submit" type="button" data-code-submit ${unlocked ? "disabled" : ""} aria-label="Potvrdit k\u00f3d">\u21b5</button>
      </div>
      <div class="code-gate-status" data-code-status>${unlocked ? "K\u00d3D OV\u011a\u0158EN" : ""}</div>
    </div>
  `;
}
...
function renderedHintLine(hint, index) {
  return `<div class="hint-line"><span>${escapeHtml(`N\u00c1POV\u011aDA ${index}:`)}</span> <span data-typewriter>${escapeHtml(hint)}</span></div>`;
}
  ...
  return `
    <div class="hints terminal-hints">
      <div class="hint-output" data-hint-output>
        ${list.slice(0, usage.text).map((hint, index) => renderedHintLine(hint, index + 1)).join("")}
      </div>
      <div class="hint-actions">
...
function bindSlide(slide) {
  ...
  root.querySelectorAll("[data-audio-pause]").forEach((btn) => {
    btn.addEventListener("click", () => toggleAudioMessagePause(btn));
  });

  root.querySelectorAll("[data-audio-scrub]").forEach((input) => {
    input.addEventListener("input", () => seekAudioMessage(input));
  });

  root.querySelectorAll("[data-code-trigger]").forEach((btn) => {
    btn.addEventListener("click", () => revealCodeGate(btn));
  });

  root.querySelectorAll("[data-code-submit]").forEach((btn) => {
    btn.addEventListener("click", () => submitCodeGate(slide, btn));
  });

  root.querySelectorAll("[data-code-input]").forEach((input) => {
    input.addEventListener("input", () => syncInputUppercase(input));
    input.addEventListener("keydown", (event) => {
      if (event.key === "Enter") {
        event.preventDefault();
        root.querySelector("[data-code-submit]")?.click();
      }
    });
  });
...
function revealCodeGate(button) {
  const gate = button.closest("[data-code-gate]");
  gate?.querySelector("[data-code-entry]")?.classList.remove("hidden");
  gate?.querySelector("[data-code-input]")?.focus();
}

function submitCodeGate(slide, button) {
  const gate = button.closest("[data-code-gate]");
  const input = gate?.querySelector("[data-code-input]");
  const status = gate?.querySelector("[data-code-status]");
  const value = input?.value || "";
  if (normalize(value) !== normalize(slide.requiredCode)) {
    if (status) status.textContent = "K\u00d3D NESOUHLAS\u00cd";
    beep("wrong");
    return;
  }

  if (!state.answers) state.answers = {};
  state.answers[codeGateKey(slide)] = "unlocked";
  saveState();
  gate?.classList.add("is-unlocked");
  gate?.querySelector("[data-code-trigger]")?.setAttribute("disabled", "");
  input.disabled = true;
  button.disabled = true;
  if (status) status.textContent = "K\u00d3D OV\u011a\u0158EN";
...
function submitQuestion(slide, mode) {
  ...
  if (mode === "open") {
    ...
    const ok = slide.digitAnswer
      ? onlyDigits(value) === onlyDigits(slide.digitAnswer)
      : (slide.acceptedAnswers || []).some((answer) => normalize(answer) === normalize(value));
...
function resetAudioControls(message) {
  const box = message?.box;
  box?.classList.remove("is-playing", "is-paused");
  if (message?.pauseButton) {
    message.pauseButton.disabled = true;
    message.pauseButton.textContent = "Stopnout";
  }
  if (message?.scrubber) {
    message.scrubber.disabled = true;
    message.scrubber.value = "0";
    message.scrubber.max = "100";
  }
}

function stopAudioMessage() {
  if (!activeAudioMessage) return;
  const message = activeAudioMessage;
  message.stop?.();
  if (message.frame) cancelAnimationFrame(message.frame);
  resetAudioControls(message);
  activeAudioMessage = null;
}

function finishAudioMessage(message, statusText = "Z\u00e1znam p\u0159ipraven k op\u011btovn\u00e9mu p\u0159ehr\u00e1n\u00ed") {
  if (message.frame) cancelAnimationFrame(message.frame);
  message.box?.classList.remove("is-playing", "is-paused");
  if (message.status) message.status.textContent = statusText;
  if (message.pauseButton) {
    message.pauseButton.disabled = true;
    message.pauseButton.textContent = "Stopnout";
  }
  ...
  if (activeAudioMessage === message) activeAudioMessage = null;
  message.context?.close?.();
}

function updateAudioScrubber(message) {
  if (!message?.audio || !message.scrubber) return;
  const duration = Number.isFinite(message.audio.duration) ? message.audio.duration : 0;
  if (duration > 0) {
    message.scrubber.max = String(duration);
    if (!message.scrubber.matches(":active")) {
      message.scrubber.value = String(message.audio.currentTime || 0);
    }
  }
}

function toggleAudioMessagePause(button) {
  const message = activeAudioMessage;
  if (!message || message.pauseButton !== button || !message.audio) return;
  if (message.audio.paused) {
    message.audio.play().then(() => {
      message.box?.classList.remove("is-paused");
      message.box?.classList.add("is-playing");
      button.textContent = "Stopnout";
      if (message.status) message.status.textContent = "P\u0159ehr\u00e1v\u00e1m vzkaz";
    }).catch(() => {
      if (message.status) message.status.textContent = "P\u0159ehr\u00e1v\u00e1n\u00ed se nepoda\u0159ilo spustit.";
    });
    return;
  }
  message.audio.pause();
  message.box?.classList.add("is-paused");
  message.box?.classList.remove("is-playing");
  button.textContent = "Pokra\u010dovat";
  if (message.status) message.status.textContent = "Vzkaz pozastaven";
}

function seekAudioMessage(input) {
  const message = activeAudioMessage;
  if (!message || message.scrubber !== input || !message.audio) return;
  const target = Number(input.value);
  if (Number.isFinite(target)) message.audio.currentTime = target;
}

function playAudioMessage(button) {
  if (activeAudioMessage?.button === button) {
    stopAudioMessage();
  } else {
    stopAudioMessage();
  }
  ...
  const pauseButton = box?.querySelector("[data-audio-pause]");
  const scrubber = box?.querySelector("[data-audio-scrub]");
  ...
  if (pauseButton) {
    pauseButton.disabled = !audioSrc;
    pauseButton.textContent = "Stopnout";
  }
  if (scrubber) {
    scrubber.disabled = !audioSrc;
    scrubber.value = "0";
    scrubber.max = "100";
  }
  ...
  activeAudioMessage = message;

  const finish = () => finishAudioMessage(message);
  ...
  if (audioSrc) {
    ...
    audio.addEventListener("loadedmetadata", () => updateAudioScrubber(message));
    audio.addEventListener("timeupdate", () => updateAudioScrubber(message));
    ...
    audio.play().catch(() => {
      if (status) status.textContent = "P?ehr?v?n? se nepoda?ilo spustit.";
      finishAudioMessage(message, "P?ehr?v?n? se nepoda?ilo spustit.");
    });
  } else {
    ...
    syllables.forEach(([offset, length, pitch, volume], index) => {
      ...
      buzz.frequency.setValueAtTime(pitch / 2, startAt);
      voiceGain.gain.setValueAtTime(0.001, startAt);
      voiceGain.gain.exponentialRampToValueAtTime(volume, startAt + 0.035);
      voiceGain.gain.setValueAtTime(volume * 0.72, Math.max(startAt + 0.04, stopAt - 0.055));
  ...
  if (canvas) message.frame = drawAudioWave(canvas, analyser);
}
